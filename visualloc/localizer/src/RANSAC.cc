...
float RANSAC::evaluate_correspondece_error( const std::vector< float > &c1, const std::vector< float > &c2 )
{
	switch(computation_type){
		
	case P6pt:{
		Util::CorrSolver::Vector2D p2d( c1[0], c1[1]);
		Util::CorrSolver::Vector3D p3d( c2[0], c2[1], c2[2] );

		return solverP6pt.evaluateCorrespondence(p2d, p3d );
